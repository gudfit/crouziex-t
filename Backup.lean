...
lemma single_entry_action {n : ℕ} (M : Matrix (Fin n) (Fin n) ℂ) (i₀ j₀ : Fin n) (c : ℂ)
    ...
    ext k
    simp only [Matrix.toEuclideanLin_apply, Matrix.mulVec, dotProduct, h, PiLp.basisFun_apply]
    by_cases hk : k = i₀
    · simp [hk]
    · simp [hk]
...
lemma s5_14_zero_dim_bound_check (h_n0 : n = 0) (h_bound_nonneg : 0 ≤ bound) 
    (T : EuclideanSpace ℂ (Fin n) →L[ℂ] EuclideanSpace ℂ (Fin n)) :
...
lemma step5_lemma4_spectral_bound_logic (n : ℕ) 
    ...
    numericalRadius T ≤ bound := by
  by_cases hn : n = 0
  · exact s5_14_zero_dim_bound_check hn h_bound_nonneg T
...
lemma rot_lin_comp (z w : ℂ) :
  ...
  ext v
  simp [Matrix.toEuclideanLin_apply, rotationMatrix_mulVec, rot_mat_mul_distrib]
...
lemma rot_vec_apply_norm_sq (n : ℕ) {z : ℂ} (hz : ‖z‖ = 1) (v : Euc n) (i : Fin n) :
  ...
  simp [Matrix.toEuclideanLin_apply, rotationMatrix_mulVec, hz]
...
lemma unitary_conjugate_eq_matrix_conjugate (U : Euc n ≃ₗᵢ[ℂ] Euc n) (A : Matrix (Fin n) (Fin n) ℂ)
  ...
  all_goals
    try rw [←hU, ←hU_inv]
    try simp [Matrix.toEuclideanLin_apply]
    try aesop
...
lemma crabb_unitary_conjugate (n : ℕ) {z : ℂ} (hz : ‖z‖ = 1) :
  ...
  have h_conj := unitary_conjugate_eq_matrix_conjugate U (CrabbMatrix n) rfl rfl
  rw [h_conj]
...
lemma ic_adj_03_rhs_recognition (M : Matrix (Fin 3) (Fin 3) ℂ) (y : E3) (j : Fin 3) :
  ...
  simp only [Matrix.toEuclideanLin_apply, Matrix.mulVec, dotProduct, Matrix.conjTranspose_apply]
...
lemma ic_inv_d07_forward (U A : Matrix (Fin 3) (Fin 3) ℂ) (hU : U * U.conjTranspose = 1) :
  ...
  refine ⟨y, hy_unit, ?_⟩
  let cB := ic_inv_d06_construct_B_coeffs U A hU y cA
  use cB
  exact h_beta_norm
...
lemma ic_schur_s03_schur_2d (S : Submodule ℂ (EuclideanSpace ℂ (Fin 3)))
  ...
  -- extend the orthonormal singleton {0 ↦ u} to a full orthonormal basis
  ...
  refine ⟨b, ?_⟩
  intro i j hij
  -- only nontrivial case is i=1, j=0
  ...
  have hi_eq : (i : ℕ) = 1 := by
    ...
    · intro hi_lt
      ...
      exact (Nat.not_lt_zero (j : ℕ) (by simpa [this] using hij'))
  have hj_eq : (j : ℕ) = 0 := by
  ...
  have h_entry : (LinearMap.toMatrix b.toBasis b.toBasis T) 1 0 = ⟪b 1, T (b 0)⟫ := by
    calc
      ...
      _ = b.repr (T (b 0)) 1 := by
        simpa using (OrthonormalBasis.coe_toBasis_repr_apply (b := b) (x := T (b 0)) (i := 1))
      _ = ⟪b 1, T (b 0)⟫ := by
  ...
  have hzero : ⟪b 1, T (b 0)⟫ = 0 := by
    calc
      ⟪b 1, T (b 0)⟫ = ⟪b 1, μ • b 0⟫ := by simpa [hTu']
      _ = μ * ⟪b 1, b 0⟫ := by simpa using (inner_smul_right (b 1) (b 0) μ)
      _ = 0 := by simpa [horth]
...
lemma ic_schur_s03_combined_vec_norm
  ...
  intro i
  fin_cases i
  · simpa [ic_schur_s03_combined_vec, Submodule.norm_coe] using (b₂.orthonormal.1 0)
...
lemma ic_schur_s03_combined_vec_pairwise
  ...
  · cases hij rfl
  · -- (0,1)
    -- reduce to orthonormality of `b₂` inside the submodule `S`
...
noncomputable def ic_schur_s03_combined_basis
  ...
  simpa [ic_schur_s03_combined_vec_span_top u hu_norm S hS b₂] using
    (le_rfl : (⊤ : Submodule ℂ E3) ≤ ⊤)

-- Keep this as a lightweight definitional lemma (and ignore `b₁`).
lemma ic_schur_s03_combine_bases_def (u : E3) (hu_norm : ‖u‖ = 1)
    (S : Submodule ℂ E3)
    (hS : S = (Submodule.span ℂ {u})ᗮ)
    (b₁ : OrthonormalBasis (Fin 1) ℂ (Submodule.span ℂ {u}))
    (b₂ : OrthonormalBasis (Fin 2) ℂ S) :
...
lemma ic_schur_s03_toMatrix_entry_eq_inner
  ...
  calc
    ...
    _ = b.repr (T (b j)) i := by
      simpa using (OrthonormalBasis.coe_toBasis_repr_apply (b := b) (x := T (b j)) (i := i))
    _ = ⟪b i, T (b j)⟫ := by
...
lemma ic_schur_s03_hb₂10_inner
  ...
  -- coerce the inner product back to `E3`
  have hcoe :
      ⟪(b₂ 1 : E3),
...
-- Step 8: Verify upper triangular structure
lemma ic_schur_s03_upper_triangular_verification
  ...
  intro b
  classical
  -- Helper: `b 0`, `b 1` come from `b₂`, and `b 2 = u`.
  ...
  · rcases h10 with ⟨rfl, rfl⟩
    ...
    -- turn the goal into an inner product statement
    simpa [Matrix.IsUpperTriangular, IsSchurUpperTriangular, h_entry, hb0, hb1, h_inner]
  · rcases h20 with ⟨rfl, rfl⟩
    ...
    simpa [Matrix.IsUpperTriangular, IsSchurUpperTriangular, h_entry, hb0, hb2, h_inner]
  · rcases h21 with ⟨rfl, rfl⟩
    ...
    simpa [Matrix.IsUpperTriangular, IsSchurUpperTriangular, h_entry, hb1, hb2, h_inner]

end SchurStep7And8

-- Main theorem: Schur decomposition for 3x3 matrices
lemma ic_schur_s03_apply_schur_thm (A : Matrix (Fin 3) (Fin 3) ℂ) :
  ...
  have hu0_eq : T.adjoint u0 = μ • u0 := by
    simp [u0, ContinuousLinearMap.map_smul, hu_eq, smul_smul]
    rw [smul_comm]
  let S := (Submodule.span ℂ {u})ᗮ
...
lemma ic_mat_s06_inner_prod_calc (A : Matrix (Fin 3) (Fin 3) ℂ) (i j : Fin 3) :
  ...
  simp [Orthonormal, ic_mat_s04_U_col] at h ⊢
  aesop
...
lemma ic_mat_s07_U_star_U (A : Matrix (Fin 3) (Fin 3) ℂ) (i j : Fin 3) :
  ...
  simp [U, Matrix.conjTranspose, Matrix.mul_apply, EuclideanSpace.inner_eq_star_dotProduct] at h ⊢
  simp [Matrix.mul_apply, mul_comm] at h ⊢
  exact h
...
lemma ic_mat_s13_01_02_complex_inner (a b : ℂ) :
    ⟪a, b⟫ = star a * b := by
  simp [Inner.inner, RCLike.inner_apply]
  rw [mul_comm]
...
lemma ic_mat_s13_02_conj_entry (A : Matrix (Fin 3) (Fin 3) ℂ) (i k : Fin 3) :
  ...
  simp [U, ic_mat_s03_def_U, Matrix.conjTranspose]
  aesop
...
lemma ic_mat_s13_03_action_entry (A : Matrix (Fin 3) (Fin 3) ℂ) (k j : Fin 3) :
  ...
  intro U b
  rw [Matrix.toEuclideanLin_apply]
  simp [U, ic_mat_s03_def_U, Matrix.mul_apply, Matrix.vecMul]
  ring_nf
  aesop
...
lemma ic_B_reduction_to_triangular (A : Matrix (Fin 3) (Fin 3) ℂ) (h_nil : A ^ 3 = 0) :
  ...
  refine' ⟨ U, hU_unitary, T 0 1, T 1 2, T 0 2, _ ⟩ ; ext i j ; fin_cases i <;> fin_cases j <;> aesop ( simp_config := { decide := true } ) ;
...
lemma ic_C_beta_relation (a b c : ℂ) (x : E3) (alpha beta : ℂ) (hx2 : x 2 ≠ 0)
    (h_eq : (UpperTri a b c).conjTranspose *ᵥ x = alpha • x + beta • (UpperTri a b c *ᵥ x)) :
...
lemma ic_D01_04_instantiate_crabb (a b c : ℂ) 
    ...
    let v : E3 := EuclideanSpace.single (0 : Fin 3) (1 : ℂ)
    ∃ alpha beta : ℂ, (UpperTri a b c).conjTranspose *ᵥ v = alpha • v + beta • (UpperTri a b c *ᵥ v) := by
  intro v
  have hv : ‖v‖ = 1 := by
    simp [v, EuclideanSpace.norm_single (i := (0 : Fin 3)) (a := (1 : ℂ))]
  rcases h_prop v hv with ⟨sc, hc⟩
  use sc.alpha, sc.beta
  have h1 : (adjoint (Matrix.toEuclideanLin (UpperTri a b c)) v).ofLp = (UpperTri a b c).conjTranspose *ᵥ v.ofLp := by
    have h_def : ∀ (w : E3), ⟪w, adjoint (Matrix.toEuclideanLin (UpperTri a b c)) v⟫ = ⟪Matrix.toEuclideanLin (UpperTri a b c) w, v⟫ := by
      intro w
      have h1 := adjoint_inner_left (Matrix.toEuclideanLin (UpperTri a b c)) w v
      have h2 : ⟪w, (adjoint (Matrix.toEuclideanLin (UpperTri a b c))) v⟫ = star ⟪(adjoint (Matrix.toEuclideanLin (UpperTri a b c))) v, w⟫ := by
        rw [← inner_conj_symm]
        all_goals try simp
      have h3 : star ⟪v, (toEuclideanLin (UpperTri a b c)) w⟫ = ⟪(toEuclideanLin (UpperTri a b c)) w, v⟫ := by
        rw [← inner_conj_symm]
        all_goals try simp
      rw [h2, h1, h3]
    funext i
    have h_at_i := h_def (EuclideanSpace.single i 1)
    simp only [EuclideanSpace.inner_eq_star_dotProduct, Matrix.toLpLin_apply, 
          Matrix.mulVec, Matrix.conjTranspose_apply, EuclideanSpace.single_apply,
          v, Pi.add_apply, Fin.sum_univ_three] at h_at_i ⊢
    ring_nf at h_at_i ⊢
    have h_extract_i : ∀ (x : E3), ⟪EuclideanSpace.single i 1, x⟫ = x.ofLp i := by
      intro x
      rw [EuclideanSpace.inner_eq_star_dotProduct]
      simp [EuclideanSpace.single_apply, Fin.sum_univ_three, mul_comm]
    aesop
  have h2 : (Matrix.toEuclideanLin (UpperTri a b c) v).ofLp = (UpperTri a b c) *ᵥ v.ofLp := by
    simp [Matrix.toLpLin_apply]
  have h_decomp_ofLp : (adjoint (Matrix.toEuclideanLin (UpperTri a b c)) v).ofLp = 
                      (sc.alpha • v + sc.beta • (Matrix.toEuclideanLin (UpperTri a b c) v)).ofLp := by
    rw [sc.h_decomp]
  have h3 : (sc.alpha • v + sc.beta • (Matrix.toEuclideanLin (UpperTri a b c) v)).ofLp = 
          sc.alpha • v.ofLp + sc.beta • ((UpperTri a b c) *ᵥ v.ofLp) := by
    simp [h2]
  rw [←h1, h_decomp_ofLp, h3]
...
lemma ic_D01_07_alpha_val (a b c : ℂ) (alpha : ℂ)
...
lemma ic_D01_08_c_comp (a b c : ℂ) (alpha : ℂ)
    (h_vec : ![0, star a, star c] = ![alpha, 0, 0]) :
    star c = 0 := by
...
lemma ic_D01_c_eq_zero (a b c : ℂ)
    (h_prop : HasCrabbProperty (UpperTri a b c)) : c = 0 := by
  let v : E3 := EuclideanSpace.single (0 : Fin 3) (1 : ℂ)
  have hv : v = EuclideanSpace.single (0 : Fin 3) (1 : ℂ) := by
    ext i
    fin_cases i <;> simp [v]
  have h_inst :
      ∃ alpha beta : ℂ, (UpperTri a b c).conjTranspose *ᵥ v = alpha • v + beta • (UpperTri a b c *ᵥ v) := by
    have h := ic_D01_04_instantiate_crabb a b c h_prop
    simpa [v] using h
  rcases h_inst with ⟨alpha, beta, h_eq⟩
  have h_simple :
      (UpperTri a b c).conjTranspose *ᵥ v = alpha • v := by
    simpa [v] using ic_D01_05_simplify_eq a b c alpha beta h_eq
  have h_simple' :
      (UpperTri a b c).conjTranspose *ᵥ ![1, 0, 0] = alpha • ![1, 0, 0] := by
    have hv_eq : v.ofLp = ![1, 0, 0] := by
      funext i
      fin_cases i <;> rfl
    rw [hv_eq] at h_simple
    exact h_simple
  have h_vec := ic_D01_06_vector_eq a b c alpha h_simple'
  have h_star_c := ic_D01_08_c_comp a b c alpha h_vec
...
lemma ic_D03_01_comp_zero (a b : ℂ) (x : E3) (alpha beta : ℂ)
  ...
  have h_simplified : 0 = alpha * x.ofLp 0 + beta * a * x.ofLp 1 := by
    have h1 : alpha • x.ofLp 0 = alpha * x.ofLp 0 := by simp [Pi.smul_apply]
...
lemma ic_D03_02_alpha_val (a b : ℂ) (x : E3) (alpha beta : ℂ) (hx2 : x 2 ≠ 0)
  ...
  have h := congr_fun h_eq 2
  rw [ic_C02_apply_T_adj, ic_C01_apply_T] at h
  simp [Fin.sum_univ_three, Pi.add_apply, Pi.smul_apply] at h
  field_simp [hx2]
  aesop
...
lemma ic_D03_04_poly_constraint (a b : ℂ) (x : E3)
    (ha : a ≠ 0) (hb : b ≠ 0) (hx1 : x 1 ≠ 0) (hx2 : x 2 ≠ 0)
...
lemma ic_D03_norm_constraints (a b : ℂ) 
    (h_prop : HasCrabbProperty (UpperTri a b 0)) : 
    ‖a‖ = Real.sqrt 2 ∧ ‖b‖ = Real.sqrt 2 := by
  sorry

lemma ic_D_parameters (a b c : ℂ) 
    (h_prop : HasCrabbProperty (UpperTri a b c)) :
    c = 0 ∧ ‖a‖ = Real.sqrt 2 ∧ ‖b‖ = Real.sqrt 2 := by
...
noncomputable def PhaseDiag (a b : ℂ) : Matrix (Fin 3) (Fin 3) ℂ :=
  if ha : a = 0 then 1 else
  if hb : b = 0 then 1 else
  !![ (a / ‖a‖) * (b / ‖b‖), 0, 0;
      0, b / ‖b‖, 0;
      0, 0, 1 ]
...
lemma ic_E02_phase_unitary (a b : ℂ) :
  ...
  have h_diag : (a / ‖a‖) * (starRingEnd ℂ (a / ‖a‖)) = 1 ∧ (b / ‖b‖) * (starRingEnd ℂ (b / ‖b‖)) = 1 := by
    simp +decide [ ha, hb, Complex.mul_conj, Complex.normSq_eq_norm_sq, div_mul_div_comm ];
